...
const globals = globalThis as unknown as { metatrayPool?: Pool; metatrayReadPool?: Pool };
export function pool(readOnly = false): Pool {
  const key = readOnly ? 'metatrayReadPool' : 'metatrayPool';
  if (!globals[key]) {
...
export async function getState<T>(db: Sql, key: string): Promise<T | null> {
  return (await db.query<{ value: T }>('SELECT value FROM metatray_state WHERE key=$1', [key])).rows[0]?.value ?? null;
}
export async function setState(db: Sql, key: string, value: unknown) {
  await db.query('INSERT INTO metatray_state(key,value) VALUES($1,$2::jsonb) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value', [key, JSON.stringify(value)]);
}
