...
export async function recentTicks(db: Sql, limit = 300): Promise<Tick[]> {
  const rows = (await db.query<{ data: Tick }>('SELECT data FROM metatray_events ORDER BY block_number DESC, (data->>\'logIndex\')::integer DESC LIMIT $1', [limit])).rows;
...
export async function applyBlock(db: Sql, block: BlockInput, cfg: PaperConfig, wallTime: number): Promise<boolean> {
  const existing = (await db.query<{ hash: string }>('SELECT hash FROM metatray_blocks WHERE number=$1', [block.number])).rows[0];
  if (existing) {
  ...
  for (const tick of block.ticks) await db.query(
    'INSERT INTO metatray_events(id,block_number,ts,data) VALUES($1,$2,$3,$4::jsonb) ON CONFLICT(id) DO NOTHING',
  ...
  const prediction = (await db.query<{ summary: PredictionSummary }>(
    'SELECT summary FROM metatray_predictions WHERE available_at<=$1 ORDER BY available_at DESC LIMIT 1', [wallTime])).rows[0]?.summary ?? null;
  ...
  await db.query('INSERT INTO metatray_blocks(number,hash,parent_hash,ts,paper) VALUES($1,$2,$3,$4,$5::jsonb)',
...
export async function rewind(db: Sql, ancestor: number, initialQuote: number) {
  const checkpoint = (await db.query<{ number: string; hash: string; ts: string; paper: PaperAccount }>(
    'SELECT number,hash,ts,paper FROM metatray_blocks WHERE number=$1', [ancestor])).rows[0];
  ...
  await db.query('SELECT id FROM metatray_jobs WHERE source_block>$1 ORDER BY id FOR UPDATE', [ancestor]);
  await db.query('DELETE FROM metatray_predictions WHERE id IN (SELECT id FROM metatray_jobs WHERE source_block>$1)', [ancestor]);
  await db.query("UPDATE metatray_jobs SET status='cancelled',lease_id=NULL,error='Source block orphaned' WHERE source_block>$1", [ancestor]);
  await db.query('DELETE FROM metatray_events WHERE block_number>$1', [ancestor]);
  await db.query('DELETE FROM metatray_blocks WHERE number>$1', [ancestor]);
...
export async function queueInference(db: Sql, options: { now: number; contextSeconds: number; refreshSeconds: number; quote: string }) {
  ...
  await db.query("UPDATE metatray_jobs SET status='cancelled',error='Superseded by newer input' WHERE status='queued'");
  await db.query('INSERT INTO metatray_jobs(id,input_hash,input,input_end,source_block) VALUES($1,$2,$3::jsonb,$4,$5) ON CONFLICT(input_hash) DO NOTHING',
