...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.METATRAY_MODE === 'demo') return NextResponse.json({ error: 'Demo contains no model predictions' }, { status: 404 });
  try {
    const { rows } = await pool(true).query('SELECT result FROM metatray_predictions WHERE id=$1', [id]);
