...
Updated 2026-09-15. This record distinguishes the temporal-surface work in the current source tree from the last recorded verification baseline. It does not claim access to a new token, a measured human response or a completed TRIBE model run.

## Current source change: temporal surface pipeline requires a fresh verification run

The current tree adds a separate temporal-surface record and read endpoint for every genuine ordered row of a completed TRIBE prediction. The worker retains full-precision values in private `prediction.npz`, publishes a bounded display-only payload with upstream starts/durations, and associates it with the prediction by a cascading foreign key. The browser validates its dimensions against the 20,484-vertex fsaverage5 mesh and replays the completed epoch at the upstream output timing. The fixed color scale is ±2 normalized model units. Any between-frame interpolation is visual only.

This model layer is intentionally independent from receipt pulses. Receipt pulses show newly observed confirmed market inputs; they are not cortical values. The temporal animation is the latest completed model epoch; it is not an instantaneous, causal response to the newest swap. Upstream feature input is 2 Hz, cortical output is 1 Hz, alignment includes an approximately five-second hemodynamic offset and the configured full window is 100 seconds. These are not millisecond neuron measurements.

Before calling this source revision verified, rerun the Node/TypeScript/build/browser commands and the Python contract suite below. Browser fixtures must check payload rejection, mesh/frame dimension matching, reduced-motion behavior, the fixed legend and the visible separation of market input from model output. Those fixtures establish interface behavior only; they are not real TRIBE results. A real acceptance run still requires Postgres, the persistent indexer, an authorized GPU worker, model access and the rights review described in `docs/DEPLOYMENT.md`.

## Last recorded baseline: live-feed implementation checked locally

This revision adds a dedicated `/experiment` page, a crisp `[tray]_` identity and favicon, an exact `metatray` browser title on every route, and a transaction-driven Three.js signal field. Each fresh confirmed receipt maps to a deterministic bounded surface location; buy/sell direction controls color and relative quote value controls pulse size in a deliberately narrow unit-independent range. A bounded seen-event history prevents old swaps from replaying, and rendering pauses while hidden or offscreen. Market pulses remain labelled inputs; only separately produced and validated TRIBE values color the model surface. The evidence page links MetaTray and immutable Meta/Pons sources. Live mode is the default; demo replay must be explicitly selected. Vercel is configured as one ordinary root Next.js project; Services mode and the Python worker are excluded.

For that earlier baseline, the production build and all **22 Node tests** passed. Coverage included confirmed-window bounds, ABI decoding, duplicate/removed events, wrong-chain rejection, event/block mismatch, reorg-during-read rejection, stale blocks, quiet markets, event caps, Pons factory discovery, privacy-safe setup errors, cortical-epoch market summaries, no transaction-pulse replay and bounded unit-independent pulse strength. `npm audit` reported zero known vulnerabilities across the installed dependency tree. All Python worker files passed syntax compilation. The executor did not provide pytest, FFmpeg or eSpeak NG, so the five-test Python suite and render smoke still had to run again in GitHub Actions for that revision before publication could be called verified.

The earlier production-browser story passed for desktop/mobile rendering, all nine informational routes, exact page titles, the Experiment 001 Open View target, the ASCII identity/favicon, live setup instructions, source links, native keyboard transaction activation, event updates without WebGL canvas replacement, receipt/explorer navigation, pause/outage behavior, reduced motion, static model-surface rendering with a controlled fixture, cortical-epoch selection, data/visual receipt downloads and a visible WebGL fallback. It checked horizontal overflow at 320, 390, 768, 1024 and 1440 pixels. The controlled RPC-shaped and model-shaped browser responses were test fixtures, not real on-chain observations or completed Meta inference. Generated screenshots and the browser JSON report are excluded from publication.
...
The earlier Node suite contained **10 passing tests**; the current suite has 22, as recorded above. The Python scientific-contract suite contains **5 tests**. Those tests use explicitly synthetic fixtures. They are not accuracy evaluations of TRIBE and are not observations from a live token.
...
- End-to-end publication and browser replay of a genuine temporal surface payload. This requires migration `002`, an actual completed GPU result and deployed checks of `/api/predictions/{id}/surface`; a synthetic browser fixture cannot establish it.
...
- Commercial model rights. CC BY-NC 4.0 plausibly does not permit token-promotional or other commercial model use without separate permission; the operator must obtain legal review and establish a valid basis. Configuration flags do not grant rights.
