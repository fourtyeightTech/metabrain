...
test('PostgreSQL migration, idempotent blocks, canonical parent enforcement, job queue and reorg rollback', async () => {
  ...
  const jobs = (await db.query<{ id: string; input: { ticks: { ts: number }[]; end: number } }>('SELECT id,input FROM tray_jobs')).rows;
  assert.equal(jobs.length, 1); assert.ok(jobs[0].input.ticks.every(t => t.ts <= jobs[0].input.end));
  await db.query("UPDATE tray_jobs SET status='running',lease_id='00000000-0000-4000-8000-000000000002' WHERE id=$1", [jobs[0].id]);
  await db.query("INSERT INTO tray_predictions(id,input_end,available_at,summary,result) VALUES($1,$2,$2,'{}','{}')", [jobs[0].id, at + 30001]);
  ...
  assert.equal((await db.query('SELECT * FROM tray_predictions')).rows.length, 0);
  const cancelled = (await db.query('SELECT status,lease_id FROM tray_jobs')).rows[0];
