...
def main():
    ...
    manifest = {"project": "metatray", "formatVersion": 1, "files": [
    ...
    target = ROOT / "dist" / "metatray.zip"; target.parent.mkdir(exist_ok=True)
    with zipfile.ZipFile(target, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for name, content in files:
            item = zipfile.ZipInfo(f"metatray/{name}", date_time=(2026, 9, 14, 0, 0, 0))
